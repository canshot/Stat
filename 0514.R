...
book.cc = character_count(book$texts)
book$count = as.numeric(book.cc)

#Count the number of each format
table(book$formats)

#Create histogram by format
library(ggplot2)
...
View(sentiment.merge)
qplot(sentiment.merge$sentiment, sentiment.merge$ratings)
